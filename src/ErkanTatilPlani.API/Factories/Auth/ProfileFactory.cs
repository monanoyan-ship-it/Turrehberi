...
public class ProfileFactory : IProfileFactory
{
        ...
        string firstName, string lastName, string? phone, string? address, DateTime? birthDate, string? bio)
    {
        ...
        if (string.IsNullOrWhiteSpace(firstName))
            return (false, new { error = "Validation.FirstNameRequired" }, 400);
        if (string.IsNullOrWhiteSpace(lastName))
            return (false, new { error = "Validation.LastNameRequired" }, 400);
    ...
    public async Task<(bool success, object result, int statusCode)> ChangePasswordAsync(string currentPassword, string newPassword)
    {
        ...
        if (string.IsNullOrEmpty(currentPassword))
            return (false, new { error = "Validation.CurrentPasswordRequired" }, 400);
        if (string.IsNullOrEmpty(newPassword))
            return (false, new { error = "Validation.NewPasswordRequired" }, 400);
        if (newPassword.Length < 6)
            return (false, new { error = "Validation.NewPasswordMinLength" }, 400);

        if (!PasswordHelper.VerifyPassword(currentPassword, visitor.PasswordHash))
            return (false, new { error = "Error.CurrentPasswordIncorrect" }, 400);
    ...
    public async Task<(bool success, object result, int statusCode)> UpdatePreferredLanguageAsync(string language)
    {
        ...
        if (!validLanguages.Contains(language))
            return (false, new { error = "Error.InvalidLanguageCode" }, 400);
        ...
        string fileName, string contentType, long fileLength, Stream fileStream)
    {
        ...
        if (fileLength == 0)
            return (false, new { error = "Error.NoFileSelected" }, 400);

        // Dosya boyutu kontrolu (max 2MB)
        if (fileLength > 2 * 1024 * 1024)
            return (false, new { error = "Error.FileSizeMax2MB" }, 400);

        // Dosya tipi kontrolu
        var allowedExtensions = new[] { ".jpg", ".jpeg", ".png", ".gif", ".webp" };
        var extension = Path.GetExtension(fileName).ToLowerInvariant();
        if (!allowedExtensions.Contains(extension))
            return (false, new { error = "Error.OnlyImageFilesAllowed" }, 400);
        ...
        return (true, new { avatarUrl = visitor.AvatarUrl, message = "Success.AvatarUploaded" }, 200);
    }
    ...
    public async Task<(bool success, object result, int statusCode)> DeleteAvatarAsync()
    {
        ...
        if (string.IsNullOrEmpty(visitor.AvatarUrl))
            return (false, new { error = "Error.NoProfilePhotoToDelete" }, 400);
        ...
        return (true, new { message = "Success.AvatarDeleted" }, 200);
    }
    ...
    public async Task<(bool success, object result, int statusCode)> UpdateNotificationPreferencesAsync(string preferencesJson)
    {
        ...
        return (true, new { message = "Success.NotificationPreferencesSaved" }, 200);
    }
}
